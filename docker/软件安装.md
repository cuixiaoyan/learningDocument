...
```shell
# 拉取镜像，官网版本的不带管理界面，安装这个比较方便
docker pull docker.io/rabbitmq:3.8-management
# 启动镜像
docker run --name cxyRabbitmq -d -p 15672:15672 -p 5672:5672 587380cbba10
```

<img src="https://gitee.com/cuixiaoyan/uPic/raw/master/uPic/image-20200905191609521.png" alt="image-20200905191609521" style="zoom:50%;" />
...
# 启动
docker run --name cxyZipkin -d -p 9411:9411 openzipkin/zipkin
```

<img src="https://gitee.com/cuixiaoyan/uPic/raw/master/uPic/image-20200906083946587.png" alt="image-20200906083946587" style="zoom:50%;" />
...
# 启动并挂载
docker run --name cxySeata-server -e SEATA_IP=192.168.106.129 -d -p 8091:8091 seataio/seata-server:1.0.0
# 修改配置
docker exec -it 镜像id /bin/sh
# 修改默认名称，和db数据库连接。数据库文件，容器里面没有，需要到官网下载。
```

<img src="https://gitee.com/cuixiaoyan/uPic/raw/master/uPic/image-20200913100727812.png" alt="image-20200913100727812" style="zoom:50%;" />
...
# 启动
docker run --name cxyNetdata -d --cap-add SYS_PTRACE -v /usr/local/docker/netdata/proc:/host/proc:ro  -v /usr/local/docker/netdata/sys:/host/sys:ro  -p 19999:19999 --restart unless-stopped netdata/netdata
# 汉化文件在上层 netdata-cpatch 中，移动到指定目录。
[root@centos8 Sinicization]# docker cp ./dashboard_info.js 537f44f4418f:/usr/share/netdata/web/
[root@centos8 Sinicization]# docker cp ./dashboard.js 537f44f4418f:/usr/share/netdata/web/
[root@centos8 Sinicization]# docker cp ./index.html 537f44f4418f:/usr/share/netdata/web/
[root@centos8 Sinicization]# docker cp ./main.js 537f44f4418f:/usr/share/netdata/web/
# 不需要重启，但是需要清理浏览器的缓存。
  
```

![image-20200925150851672](https://gitee.com/cuixiaoyan/uPic/raw/master/uPic/image-20200925150851672.png)
